...
| `.bulk_delete_entries(item_ids)` | `→ Dict` | [whitelist_service.py:1009](../../../server/services/whitelist_service.py#L1009) | Canonical bulk delete; vẫn hỗ trợ fallback pseudo-ID `group::<gid>::<type>::<value>` cho legacy embedded rows. `deleted_count` đếm theo request khớp (`len(matched)` cho group, +1/id cho global) → không vượt số `item_ids` |
...
| `_merge_whitelists(global_entries, group_entries)` | | [whitelist_service.py:747](../../../server/services/whitelist_service.py#L747) | Merge theo key `type:value`. **Group entry thắng** global (order-independent qua tie-break scope). Preserve priority="high" từ cả 2 phía |
